...
\begin{itemize}
    ...
    However, our results on these splits diverged significantly from the original paper's benchmarks. 
    We attribute this degradation to a lack of exhaustive hyperparameter optimization.
    Due to strict compute limitations on our remote server with the majority of our allocated GPU hours consumed by the extensive 100-hour baseline run. 
    We were unable to execute the large-scale parameter sweeps necessary to stabilize convergence in these extreme low-data cases.
\end{itemize}
