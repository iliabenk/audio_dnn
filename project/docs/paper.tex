...
\begin{figure}[htbp]
    \centering
    \textcolor{red}{\textbf{[TODO: We need to refactor and make sure this section is accurate]}}
\end{figure}
...
Our experiments are based on the 95-million parameter ``BASE'' HuBERT model \cite{hubert} implemented using the Hugging Face \texttt{transformers} library. For our fine-tuning pipeline, the architecture is divided into three functional stages:
...
To handle varying hardware environments, we modularized our setup via YAML configuration files (e.g., \texttt{libri-100h.yaml}, \texttt{mac.yaml}, \texttt{debug.yaml}).
