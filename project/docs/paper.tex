...
\begin{figure}[htbp]
    ...
    \caption{HuBERT pre-training method. An external Acoustic Unit Discovery System assigns discrete targets ($z_i$) for each frame based on raw audio. The model, comprising a CNN encoder and a Transformer, down-samples raw audio and applies masking to contiguous frame spans ($[\text{MSK}]$). Solid blue arrows visualize that a cross-entropy loss is computed on the Transformer's predictions for only the masked frame positions, against their respective assigned targets, forcing the model to infer them from temporal context. (Red dashed lines indicate parts of the external clustering pipeline). Modified from \cite{hubert}.}
